...
export default async function handler(request, response) {
  ...
  if (!config.searchKey || !process.env.TYPESENSE_HOST) {
    return sendJson(response, 503, { error: "Typesense is not configured" });
  }

  const url = new URL(request.url, `https://${request.headers.host || "localhost"}`);
  const species = url.searchParams.get("species") === "dog" ? "dog" : "cat";
  const collection = species === "cat"
    ? process.env.CAT_TYPESENSE_COLLECTION || "cats"
    : process.env.DOG_TYPESENSE_COLLECTION || config.collection || "dogs";
  ...
  try {
    const result = await fetch(`${config.base}/collections/${collection}/documents/search?${query}`, {
      headers: { "X-TYPESENSE-API-KEY": config.searchKey },
    });
    const payload = await result.json();
    if (!result.ok) return sendJson(response, result.status, { error: payload.message || "Pet search failed" });
    return sendJson(response, 200, { source: "typesense", species, radius, found: payload.found, page, hits: payload.hits });
  } catch {
